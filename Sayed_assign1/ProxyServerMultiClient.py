...
def proxy_handler(tcpCliSock, addr):
    ...
    # Lock shared resource(dictionary)
    lock = threading.Lock()
    lock.acquire()
    ...
    # Release shared resource(dictionary)
    lock.release()
    
    # Cache hit happens
    if hit:
        print('Flie exists in cache.')
...
def main():
    ...
    # Run infinite loop for receiving multipple clients request.
    while True:
        ...
        # Dispatching client request(browser) to proxy thread and waiting for new client requrest.
        thread = threading.Thread(target = proxy_handler, args = (tcpCliSock, addr))
